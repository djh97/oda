...
def validate_generation_manifest(output_dir: Path = DATASET_DIR) -> List[str]:
    ...
    recorded_protocol = source_inputs.get("protocol_json", {})
    inherited_protocol = (
        manifest.get("protocol_version") != PROTOCOL["version"]
        or not isinstance(recorded_protocol, Mapping)
        or recorded_protocol.get("sha256") != _sha256(DEFAULT_PROTOCOL_PATH)
    )
        ...
        try:
            lineage = assert_preserved_artifact_lineage(
                ...
                protocol_source_sha256=(
                    recorded_protocol.get("sha256")
                    if isinstance(recorded_protocol, Mapping)
                    else None
                ),
            )
            generator_source = lineage.get("preserved_sources", {}).get(
                "synthetic_dataset_generator"
            )
            generator_record = source_inputs.get("generator", {})
            if (
                not isinstance(generator_source, Mapping)
                or not isinstance(generator_record, Mapping)
                or generator_source.get("sha256") != generator_record.get("sha256")
            ):
                errors.append("Generation manifest generator lineage is invalid")
    ...
    if inherited_protocol:
        expected_sources.pop("generator")
